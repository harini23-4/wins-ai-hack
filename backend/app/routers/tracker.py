...
def list_projects(owner_profile_id: Optional[UUID] = None, db: Session = Depends(get_db)):
...
def project_summary(project_id: UUID, db: Session = Depends(get_db)):
...
def update_milestone(milestone_id: UUID, payload: schemas.MilestoneUpdate, db: Session = Depends(get_db)):
    m = db.get(models.Milestone, milestone_id)
    if not m:
        raise HTTPException(404, "Milestone not found")
...
def delete_publication(pub_id: UUID, db: Session = Depends(get_db)):
    p = db.get(models.Publication, pub_id)
    if not p:
        raise HTTPException(404, "Publication not found")
    db.delete(p)
...
def get_funding(funding_id: UUID, db: Session = Depends(get_db)):
...
def delete_funding(funding_id: UUID, db: Session = Depends(get_db)):
    f = db.get(models.FundingSecured, funding_id)
    if not f:
        raise HTTPException(404, "Funding entry not found")
    db.delete(f)
